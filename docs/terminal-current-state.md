...
- **开发环境**：Mise (唯一管理 Node.js/Bun/pnpm 的版本与切换)

## 核心工具集
| 功能 | 工具 | 说明 |
|---|---|---|
| **Prompt** | `starship` | 极简、上下文感知的提示符 |
| **历史记录** | `atuin` | `Ctrl+R` 触发，保留原生上下键 |
| **模糊搜索** | `fzf` | `Ctrl+T` 选择文件、`Alt+C` 选择目录，不占用 `Ctrl+R` |
...
## 管理策略
- **Home Manager/Nix**：安装并配置 Ghostty、Fish、WezTerm、Zsh、Starship、Git、Helix、tmux、direnv/nix-direnv 与通用 CLI；终端应用自身更新检查关闭。
...
## 设计与主题约束
1. **单一目标所有者**：Ghostty 与 WezTerm 的软件、静态配置与主题，以及 Fish/Zsh 配置均以 `nix-config` 为唯一声明来源；本仓库的 YAML 继续服务尚未迁移的消费者。
...
6. **快捷入口**：终端、Shell 与 CLI 快捷键总表位于 `nix-config/docs/guide/SHORTCUTS.md`。
